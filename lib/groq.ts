...
export async function generateRevisionPartWithGroq(input: {
  ...
  const verification = await chat(
    [
      ...
      "Retourne UNIQUEMENT les encarts à insérer. S'il n'y a vraiment aucun élément sensible au temps dans le brouillon, retourne exactement NO_UPDATE.",
      "Dès qu'un brouillon contient au moins un nom d'offre, de plan, de version, de rôle, de permission, de fonctionnalité, de procédure, de terminologie officielle, de montant, de taux, de seuil, de plafond, de délai ou de date, tu DOIS produire au moins un bloc :::update ou :::current après vérification primaire ; NO_UPDATE est alors interdit.",
