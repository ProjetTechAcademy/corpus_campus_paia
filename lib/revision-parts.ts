...
export async function prepareRevisionParts(resourceCode: string, groupsPerPart = 3) {
  ...
  await getPool().query(
    `
      ...
      SET content='', status='pending', attempts=0, last_error=NULL, generated_at=NULL, format_version=6, updated_at=now()
      WHERE resource_code=$1 AND format_version < 6
    `,
    [resourceCode],
  );
...
export async function completeRevisionPart(resourceCode: string, partIndex: number, content: string) {
  ...
  await getPool().query(
    `
      UPDATE campus_paia.resource_revision_parts
      SET content=$3, status='done', last_error=NULL, generated_at=now(), format_version=6, updated_at=now()
      WHERE resource_code=$1 AND part_index=$2
    `,
